...
import { CATEGORIAS_DOCUMENTO, isValidFileType, isValidFileSize, FormValidator } from './utils.js'
  ...
  const handleFileUpload = (event) => {
    ...
    try {
      ...
      if (!isValidFileSize(file.size, maxSize)) {
        setUploadMessage({ 
          type: 'error', 
          text: `Arquivo muito grande. Máximo permitido: 10MB (seu arquivo: ${(file.size / 1024 / 1024).toFixed(2)}MB)` 
        })
        return
      }
      ...
      if (!isValidFileType(file.name, allowedTypes)) {
        setUploadMessage({ 
          type: 'error', 
          text: `Tipo de arquivo não permitido. Aceitos: ${allowedTypes.join(', ')}` 
        })
        return
      }
      ...
      reader.onload = (e) => {
        try {
          ...
          setUploadMessage({
            type: 'error',
            text: 'Erro ao processar arquivo. Tente novamente.'
          })
        }
      }
      reader.onerror = () => {
        setUploadMessage({
          type: 'error',
          text: 'Erro ao ler arquivo. Tente novamente.'
        })
      }
      ...
      setUploadMessage({
        type: 'error',
        text: 'Erro ao validar arquivo. Tente novamente.'
      })
    }
